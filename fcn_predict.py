...
def output_result1(r=[],imgs=[],namout='test_xyz.txt'):
     ...
     dataout1=[xyz[i][0:4]+[xyz[i][0],xyz[i][1]] for i in range(0,len(xyz))]
     f=open(namout,'w')
...
def predict():
    ...
    r=[[39,0.1,160],[-135,0.1,150],[-5,2,40]]
    wave_test,loca_true=sgydata.load_sgylist_xyz1(shuffle='false',sgylist=['./waveform_data/','testing_samples.txt'],
                                sgyr=[0,-1,1],xr=r[0],yr=r[1],zr=r[2],r=0.5,shiftdata=[list(range(20,50))+list(range(-200,-20)),0])
    loca_true=np.reshape(loca_true,(len(loca_true),160,150,40)) 
